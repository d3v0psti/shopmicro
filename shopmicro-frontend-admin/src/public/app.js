...
const isAdminToken = token => {
  try {
    ...
    return role === 'Admin' && payload.identity_scope === 'administration';
  } catch {
    return false;
  }
};
...
const loadUsers = async () => {
  try {
    const [customersResponse, adminsResponse] = await Promise.all([
      fetchWithAuth(`${window.location.origin}/api/administration/marketplace-accounts`),
      fetchWithAuth(`${window.location.origin}/api/administration/accounts`)
    ]);
...
const renderUserList = (users, target, userType) => {
  ...
  users.forEach(user => {
    ...
    resetButton.onclick = () => resetUserPassword(user.email, userType);

    const deleteButton = document.createElement('button');
    deleteButton.textContent = 'Excluir';
    deleteButton.className = 'btn-primary';
    deleteButton.style.background = '#dc2626';
    deleteButton.onclick = () => deleteUser(user.email, userType);
...
const resetUserPassword = async (email, userType) => {
  const newPasswordValue = prompt(`Informe a nova senha para ${email}:`, '');
  if (!newPasswordValue) return;

  try {
    const accountPath = userType === 'Admin' ? 'accounts' : 'marketplace-accounts';
    const response = await fetchWithAuth(`${window.location.origin}/api/administration/${accountPath}/${encodeURIComponent(email)}/password`, {
      method: 'PUT',
...
const deleteUser = async (email, userType) => {
  ...
  try {
    const accountPath = userType === 'Admin' ? 'accounts' : 'marketplace-accounts';
    const response = await fetchWithAuth(`${window.location.origin}/api/administration/${accountPath}/${encodeURIComponent(email)}`, {
      method: 'DELETE'
    });
...
const refreshAccessToken = async () => {
  try {
    const response = await fetch(window.location.origin + '/api/administration/auth/refresh-token', {
      method: 'POST',
...
const login = async (event) => {
  ...
  try {
    const response = await fetch(window.location.origin + '/api/administration/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'same-origin',
...
const logout = async () => {
  try {
    await fetch(window.location.origin + '/api/administration/auth/logout', {
      method: 'POST',
      credentials: 'same-origin'
    });
  } catch (error) {
...
const handleUserRegister = async (event) => {
  ...
  try {
    const response = await fetchWithAuth(window.location.origin + '/api/administration/marketplace-accounts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        email,
...
const handleAdminRegister = async (event) => {
  ...
  try {
    const response = await fetchWithAuth(`${window.location.origin}/api/administration/accounts`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const handleChangePassword = async (event) => {
  ...
  try {
    const response = await fetchWithAuth(`${window.location.origin}/api/administration/accounts/${encodeURIComponent(userEmailStored)}/password`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },

...
public class OrdersController : ControllerBase
{
        ...
        [FromQuery] string? email, CancellationToken ct)
    {
        ...
        if (User.HasClaim(IdentityScopes.ClaimName, IdentityScopes.Administration))
        {
            if (!string.IsNullOrWhiteSpace(email))
                query = query.Where(o => o.CustomerEmail.ToLower() == email.ToLower());
        }
        else if (User.HasClaim(IdentityScopes.ClaimName, IdentityScopes.Marketplace))
        {
            var currentEmail = CurrentEmail();
            query = query.Where(o => o.CustomerEmail.ToLower() == currentEmail.ToLower());
        }
        else
        {
            return Forbid();
    ...
    public async Task<ActionResult<OrderResponse>> GetById(Guid id, CancellationToken ct)
    {
        ...
        if (!CanAccess(order.CustomerEmail)) return Forbid();
    ...
    public async Task<ActionResult<OrderResponse>> Create(CreateOrderRequest request, CancellationToken ct)
    {
        ...
        if (!string.Equals(CurrentEmail(), request.CustomerEmail, StringComparison.OrdinalIgnoreCase))
            return Forbid();
    ...
    private bool CanAccess(string customerEmail) =>
        User.HasClaim(IdentityScopes.ClaimName, IdentityScopes.Administration)
        || (User.HasClaim(IdentityScopes.ClaimName, IdentityScopes.Marketplace)
            && string.Equals(CurrentEmail(), customerEmail, StringComparison.OrdinalIgnoreCase));
}

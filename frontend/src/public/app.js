...
function setupProfileModalEvents() {
  ...
  if (btnConfirmDelete) {
    btnConfirmDelete.addEventListener('click', async () => {
      ...
      try {
        // Validação: Verifica se há pedidos associados antes de deletar a conta
        const ordersRes = await fetch(`${API_URL}/api/v1/orders`).catch(() => null);
        
        if (ordersRes && ordersRes.ok) {
          const allOrders = await ordersRes.json();
          const userOrders = allOrders.filter(order => 
            (order.customerEmail || order.UserEmail || '').toLowerCase() === currentUser.email.toLowerCase()
          );

          if (userOrders.length > 0) {
            if (deleteModal) deleteModal.classList.add('hidden');
            showToast('⚠️ Você possui pedidos pendentes/ativos. Cancele-os antes de excluir a conta.', 'error');
            return;
          }
        }

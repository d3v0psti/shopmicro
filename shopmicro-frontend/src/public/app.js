...
async function fetchWithAuth(url, options = {}) {
  ...
  const refreshResponse = await fetch(`${API_URL}/api/marketplace/auth/refresh-token`, {
    method: 'POST'
  });
...
function setupAuthEvents() {
  ...
  if (btnOpenLogin) {
    btnOpenLogin.addEventListener('click', () => {
      if (currentUser) {
        fetch(`${API_URL}/api/marketplace/auth/logout`, { method: 'POST' }).catch(() => null);
        currentUser = null;
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/marketplace/auth/login`, {
          method: 'POST',
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/marketplace/accounts`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const loginResponse = await fetch(`${API_URL}/api/marketplace/auth/login`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
function setupProfileModalEvents() {
  ...
  if (editProfileForm) {
    editProfileForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetchWithAuth(`${API_URL}/api/marketplace/accounts/${encodeURIComponent(currentUser.email)}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  if (btnConfirmDelete) {
    btnConfirmDelete.addEventListener('click', async () => {
      ...
      try {
        ...
        const ordersRes = await fetchWithAuth(`${API_URL}/api/v1/orders`).catch(() => null);
...
async function loadMyOrders() {
  ...
  try {
    const res = await fetchWithAuth(`${API_URL}/api/v1/orders`);

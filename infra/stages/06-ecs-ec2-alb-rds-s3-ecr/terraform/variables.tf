...
variable "root_volume_size" {
  ...
  validation {
    ...
    error_message = "A AMI Amazon Linux 2023 otimizada para ECS exige pelo menos 30 GiB."
  }
}

variable "backend_ecr_repository" {
  description = "Repositório ECR manual e persistente da imagem do backend."
  type        = string
  default     = "shopmicro-backend"
}

variable "frontend_ecr_repository" {
  description = "Repositório ECR manual e persistente da imagem do frontend."
  type        = string
  default     = "shopmicro-frontend"
}

variable "frontend_admin_ecr_repository" {
  description = "Repositório ECR manual e persistente da imagem do frontend-admin."
  type        = string
  default     = "shopmicro-frontend-admin"
}
...
variable "ecs_instance_min_size" {
  description = "Quantidade mínima de instâncias do cluster ECS."
  type        = number
  default     = 2
}

variable "ecs_instance_max_size" {
  description = "Quantidade máxima de instâncias controladas pelo Capacity Provider."
  type        = number
  default     = 4
}

variable "service_desired_count" {
  description = "Quantidade desejada de tasks por ECS Service."
  type        = number
  default     = 2
}
